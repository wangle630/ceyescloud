        ...
        switch(arguments[0]) {
            ...
            case "saved":
                ...
                try {
                    ...
                    $('#upload_status').css({'color': '#F00'}).text("鍙戦€佸け璐�,鏈嶅姟鍣ㄨ繑鍥炴棤鏁堢殑鏁版嵁");
                    return;
                }
                if(0 == data.ret) {
                    $('#upload_status').css({'color': '#0F0'}).text("鍙戦€佹垚鍔�");
                } else {
                    $('#upload_status').css({'color': '#F00'}).text(name + "鍙戦€佸け璐�, 閿欒浠ｇ爜: " + data.ret);
                }
                //if (data.saved) {
            ...
            case "save_cancel":
                name = arguments[1];
                var errorMessage = arguments[2];
                $uploadStatus.css({'color': '#F00'}).text("鍙栨秷鍙戦€�,鍥犱负鏈寚瀹氱洰鏍囩敤鎴�" );
                break;
